  ...
  const applyJobHandler = async () => {
    try {
      ...
      if (res?.data?.success) {
        ...
        dispatch(setSingleJob(updatedSingleJob));  //heps to update UI in real time
  ...
  useEffect(() => {
    const fetchSingleJob = async () => {
      try {
        ...
        console.log("problem in single job api");
      }
    }
    fetchSingleJob();
        ...
        <div>
          ...
          <div className='flex items-center gap-2 my-4'>
            <div className='flex items-center gap-2 mt-4' >
              ...
              <Badge className={'text-[#7209B7] font-extrabold'} variant="ghost">{singleJob?.salary} LPA</Badge>
            </div>
          </div>
        </div>
        <Button
          ...
          disabled={isApplied}
          className={`rounded-lg ${isApplied ? 'bg-gray-600 cursor-not-allowed' : 'bg-[#7209b7] hover:bg-[#543b65] p-4'}`}>{isApplied ? " Already Applied" : "Apply Now"}
        </Button>
      </div>
